...
public class SentencesByLexemeLengthComparator implements Comparator<Component> {
    ...
    private static int calculateLength(Component parentComponent) {
        ...
        for (Component component : parentComponent.getChild()) {
            for (Component lexemeComponent : component.getChild()) {
                ...
                if (lexemeComponent.getType().equals(ComponentType.SYMBOL)) {
                    length.addAndGet(lexemeComponent.getChild().size());
                }
            }
        }
